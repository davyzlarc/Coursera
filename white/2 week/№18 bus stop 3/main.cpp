...
//
//Замечание
//Будьте внимательны при обновлении словаря : если одновременно в одном выражении вычислять размер словаря и добавлять
//в него новый элемент, результат может отличаться от ожидаемого на единицу.Чтобы этого избежать, используйте временную 
//переменную.
//
...
int main()
{
	map<set<string>, int>GUIDE;
	int num_guides = int(GUIDE.size())+1;
	...
	for (auto i = 1; i <= n; i++)
	{
		...
		for (auto i = 0; i < num_stops; i++)
		{
			static string item;
			cin >> item;
			tmp_stops.insert(item);
		...
		if (GUIDE.count(tmp_stops) == 0)
		{
			...
			result.push_back("Already exists for " + to_string(GUIDE.at(tmp_stops)));
	}
